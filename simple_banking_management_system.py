...
def register_user():
    ...
    while True: # Loop for phone number input and validation
        ...
        if len(ph_no) == 10 and ph_no.isdigit() and ph_no[0] in ['6', '7', '8', '9']:
            cursor.execute("SELECT ph_no FROM user WHERE ph_no = %s", (ph_no,))
    ...
    cursor.execute("INSERT INTO user (name, pin, account_type, balance,ph_no) VALUES (%s, %s, %s, %s, %s)",(name, pin, account_type, balance,ph_no))
    conn.commit()
...
def user_functions():
    while True:
        ...
        if choice == "1": # Viewdetails
            ...
            print(tab(stmt, headers=headers, tablefmt="fancy_grid")) 
            conn.commit()
        elif choice=="8":
            break
        elif choice.isalpha():
